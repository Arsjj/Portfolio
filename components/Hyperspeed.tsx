...
type ColorInput = ColorValue | ColorValue[];
...
type EffectOptions = {
  ...
  colors: {
    roadColor: ColorValue;
    islandColor: ColorValue;
    background: ColorValue;
    shoulderLines: ColorValue;
    brokenLines: ColorValue;
    leftCars: ColorValue[];
    rightCars: ColorValue[];
    sticks: ColorInput;
  };
};

type ResolvedEffectOptions = Omit<EffectOptions, 'distortion'> & {
...
const Hyperspeed = ({ effectOptions = {} }: HyperspeedProps) => {
  ...
  const appRef = useRef<DisposableApp | null>(null);
  ...
  useEffect(() => {
    ...
    const distortions: Record<DistortionKey, Distortion> = {
      mountainDistortion: {
    ...
    const random = (base: number | Range): number => {
    ...
    class CarLights {
      ...
      constructor(webgl: App, options: ResolvedEffectOptions, colors: ColorInput, speed: Range, fade: THREE.Vector2) {
        this.webgl = webgl;
        this.options = options;
        this.colors = colors;
        this.speed = speed;
    ...
    const selectedDistortion = effectOptions.distortion ?? DEFAULT_EFFECT_OPTIONS.distortion;

    const options: ResolvedEffectOptions = {
      ...DEFAULT_EFFECT_OPTIONS,
      ...effectOptions,
      colors: { ...DEFAULT_EFFECT_OPTIONS.colors, ...effectOptions.colors },
      distortion: typeof selectedDistortion === 'string' ? distortions[selectedDistortion] : selectedDistortion
    };
